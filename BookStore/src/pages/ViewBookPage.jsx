...
const ViewBook = () => {
  ...
  const getBook = async () => {
    // setLoading(true);
    try {
      axios.get(`https://www.googleapis.com/books/v1/volumes/${id}?key=AIzaSyCkS0j6hAV0oA1H4CyBVWJhk5yDN-g8KXw`)
      .then(res=>{
        setBook(res.data);
      })
    } catch (error) {
    ...
    <>
      ...
      <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <Box display="flex" flexDirection={{ xs: "column", md: "row" }} gap={4}>
          {/* Book Cover */}
          <Box flexShrink={0} sx={{ display: "flex", justifyContent: "center" }}>
            <CardMedia
              component="img"
              alt={title}
              image={imageSrc}
          ...
          <Box sx={{ flexGrow: 1 }}>
            ...
            <Typography variant="body1" color="textPrimary" >
              Price: <CurrencyRupeeIcon sx={{fontSize:'19px'}}/>{pageCount===0?655:pageCount*3}
            </Typography>
